...
describe('load function', () => {
	it('should return version info from server', async () => {
		server.use(
			http.get(mockUrl, () => HttpResponse.json({ data: { server: '1.0.0', statemint: '2.0.0', estimint: '3.0.0' } }))
		);

		const data = (await load({} as any)) as any;

		expect(data.versionInfo).toEqual({ server: '1.0.0', statemint: '2.0.0', estimint: '3.0.0' });
	});
